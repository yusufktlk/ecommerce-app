...
function Cart() {
  ...
  return (
    <div className='bg-black w-[350px] overflow-hidden overflow-y-auto h-[500px] fixed  z-40 mt-14  right-0 rounded-l-xl  '>
        <hr className='border-black border-2' />
        <h1 className='text-center text-lg font-bold mt-4 underline mb-2 '>Cart</h1>
        <div className='flex flex-col gap-y-4 items-center'>
            {
              basket.map((basket,key) => (
                <div key={key} className='flex gap-x-4 items-center text-[12px]'>
                  <h1 className='w-32'>{basket.title}</h1>
                  <h1 className='text-red-600 text-sm'>{basket.price} $</h1>
                  <h1>Adet:{basket.quantity}</h1>
